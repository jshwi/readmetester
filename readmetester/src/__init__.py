...
from typing import Union, Iterator, Any, List, Optional, Type, Dict
...
class Parenthesis:
    ...
    def eval(self, command):
        ...
        bracket = command[-1]
        if bracket in self._openers:
            self.entries.append(bracket)

        elif (
            bracket in self._closers
            and self.entries
            and self.entries[-1] == self._openers[self._closers.index(bracket)]
        ):
            self.entries.pop()

    def command_ready(self, command):
        """Boolean value for whether command is ready to execute or
        not.
        """
        return (
            not any(command.endswith(i) for i in (":", ","))
